...
def override_dependencies(fake_user):
    ...
    app.dependency_overrides[
        __import__("backend.routing.shortLinks").routing.shortLinks.get_short_link_service
    ] = lambda: mock_service

    yield mock_service
    app.dependency_overrides.clear()

def test_create_short_link(client, override_dependencies, fake_short_link):
...
def test_get_link_info(client, override_dependencies, fake_short_link):
...
def test_redirect_to_original(client, override_dependencies, fake_short_link):
    override_dependencies.redirect.return_value = fake_short_link.original_url
    response = client.get(f"/short-links/r/{fake_short_link.short_code}", follow_redirects=False)
    assert response.status_code == 307
    assert response.headers["location"] == fake_short_link.original_url

def test_verify_password_not_protected(client, override_dependencies, fake_short_link):
    fake_short_link.is_protected = False

    override_dependencies.verify_password_short_link.return_value = fake_short_link
    response = client.post(f"/short-links/verify-password?code={fake_short_link.short_code}&password=test")
    assert response.status_code == 200
    assert response.json()["status"] == "not_protected"

def test_generate_qr_for_link(client, override_dependencies, fake_short_link, mocker):
    override_dependencies.get_info.return_value = fake_short_link
    fake_task = MagicMock()
    fake_task.id = "task-123"

    mocker.patch("backend.routing.shortLinks.generate_qr_code_task.delay", return_value=fake_task)
